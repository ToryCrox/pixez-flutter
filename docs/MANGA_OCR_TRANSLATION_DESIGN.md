...
5. 启动后若“随页面自动识别翻译”开启，上下滚动并稳定停留约 350ms 后自动处理新的当前页；关闭面板即停止自动跟随。
...
连续阅读使用单任务、最新页优先的协调器。当前页由可见高度最大的图片决定，而不是下一页刚出现时立即切换；滚动稳定后才查询缓存或启动处理。helper 同时只执行一页，等待槽只保留最新页，因此快速经过第 4、5、6 页时不会为第 4、5 页创建新任务。已处理页面保存在页面会话和 SQLite 中，回看时直接显示；自动翻页不会设置 `forceOcr` 或 `forceTranslation`。
