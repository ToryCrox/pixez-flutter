...
4. 本地 OCR 完成后页面立即显示原文并释放 OCR helper；AI 翻译在独立队列后台继续，译文返回后自动更新。
...
连续阅读对本地 OCR 使用单任务、最新页优先的协调器。页数指示器仍由可见高度最大的图片决定，避免下一页刚出现时过早跳页；OCR 调度则更积极：只要下一页进入视口就立即排队，路径解析器通过同一个 Pixiv 缓存等待本地文件可读，随后直接开始 OCR，无需等待它成为当前页或完成 Flutter 绘制。单页本地 OCR 完成后会立即释放 helper，下一页可与前一页的 AI 翻译并行进行；AI 翻译独立限流，默认最多 2 个请求同时发送。等待槽只保留滚动方向上的最新可见页，因此快速经过第 4、5、6 页时不会积压全部本地 OCR 任务。已处理页面保存在页面会话和 SQLite 中，回看时直接显示；自动翻页不会设置 `forceOcr` 或 `forceTranslation`。销毁页面会话时，已发出的 AI 请求不强制中断，但其迟到响应不会回写已销毁的控制器。
