  ...
  Future<void> _loadPage({bool next = true}) async {
    ...
    try {
      ...
      for (final illust in illusts) {
        final local = await downloadStore.dbProvider.getIllustByIllustId(
          illust.id,
        );
    ...
    try {
      ...
      setState(() {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('成功同步 $successCount 个收藏状态')));
      });
    } catch (e) {
      setState(() {
          ...
          child: Row(
            children: [
              ...
              TextButton(
                onPressed:
                    _currentIllusts.any(
                          (i) => _localIllusts[i.id]?.bookmark == 0,
                        )
                        ? () {
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return CheckboxListTile(
                value: isSelected,
                onChanged:
                    (isDownloaded && !isBookmarked)
                        ? (val) {
                          setState(() {
                            if (val == true)
                              _selectedIds.add(illust.id);
                            else
                              _selectedIds.remove(illust.id);
                          });
                        }
                        : null,
  ...
  List<Widget> _buildActions() {
    if (_state == 2) {
      return [
        ...
        if (_pageIndex > 0)
          TextButton(onPressed: () => _onPageChange(false), child: Text('第一页')),
        ElevatedButton(
