...
class _NetworkPageState extends State<NetworkPage> {
  ...
  Future<void> _refreshAllDns() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('DNS 查询完成')));
    }
  }
  ...
  Future<void> _showAddCustomIpDialog(String host) async {
    ...
    if (result != null && result.isNotEmpty) {
      ...
      if (mounted && ips.isNotEmpty) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('已添加 ${ips.length} 个IP')));
      }
    }
  }
  ...
  Future<void> _saveCustomHost() async {
    ...
    if (_textEditingController.text.trim().contains(" ")) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("illegal"), backgroundColor: Colors.red),
      );
      return;
    }
  ...
  Widget _buildHostInfoCard(HostInfo info) {
    ...
    final allIps = <_IpWithSource>[
      ...info.defaultIps.asMap().entries.map(
        (e) => _IpWithSource(e.value, IpSource.defaultValue, e.key),
      ),
      ...info.dnsIps.asMap().entries.map(
        (e) => _IpWithSource(e.value, IpSource.dns, e.key),
      ),
      ...info.customIps.asMap().entries.map(
        (e) => _IpWithSource(e.value, IpSource.custom, e.key),
      ),
    ];
  ...
  Widget _buildIpChip(_IpWithSource item, HostInfo hostInfo) {
    final isSelected =
        hostInfo.selectedSource == item.source &&
        hostInfo.selectedIndex == item.index;
    final color = _getSourceColor(item.source);
    final latencyText =
        item.ipInfo.latency == null
            ? ''
    ...
    return InkWell(
      ...
      onLongPress: () {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('已复制: ${item.ipInfo.ip}')));
      },
          ...
          children: [
            ...
            if (latencyText.isNotEmpty) ...[
              ...
              Text(
                ...
                style: TextStyle(
                  fontSize: 9,
                  color: _getLatencyColor(item.ipInfo.latency),
                ),
              ),
            ],
            // 选中标记
