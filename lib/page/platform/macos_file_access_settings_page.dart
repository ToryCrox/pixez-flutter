...
class _MacOSFileAccessSettingsPageState
  ...
  Future<void> _requestAccess() async {
    try {
      ...
      if (success && path != null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('已授权访问: $path')),
          );
        }
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('用户取消或授权失败')),
          );
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('请求授权失败: $e')),
        );
      }
    }
  }
  ...
  Future<void> _clearBookmark(String path) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('已清除: $path')),
        );
      }
      await _loadBookmarks();
    } catch (e) {
  ...
  Future<void> _clearAllBookmarks() async {
    ...
    if (confirmed == true) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('已清除所有授权')),
          );
        }
        await _loadBookmarks();
      } catch (e) {
      ...
      body: _isLoading
          ...
          : Column(
              children: [
                ...
                Expanded(
                  child: _bookmarkedPaths.isEmpty
                      ? Center(
                          child: Column(
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final isExternal =
                                MacOSFileAccessManager.isExternalVolume(path);
